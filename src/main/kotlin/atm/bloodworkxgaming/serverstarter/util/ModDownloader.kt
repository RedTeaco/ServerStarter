 ...
 *
 * 两个入口：
 * - [downloadAll]：单链接（行为与历史一致：失败后整体重试一轮）；
 ...
 *
 * 落盘方式（与镜像装 loader 的 LibraryDownloadTask 对齐）：
 * 先下到 `<name>.part`，用 manifest / CF 给的哈希校验（[DownloadIntegrity]），通过后才移动到最终路径。
 * 于是「CDN 返回错文件」「下载被截断」「进程崩在下载中途」都不会在 mods/ 里留下坏 jar，
 * 哈希不符也会当作该链接失败、自动换下一个候选链接。
 */
class ModDownloader(private val basePath: String, private val internetManager: InternetManager) {

    /** 一个待下载文件：有序候选链接（第一个优先）+ 目标文件名 + 可选的清单哈希。 */
    ...
    /** 多候选链接下载：优先 `urls[0]`，失败按序回退；带哈希时校验后才落盘。 */
    ...
    private fun downloadSingle(
        ...
        val dest = File(basePath + "mods/" + modName)
        val temp = File(dest.parentFile, modName + PART_SUFFIX)
        var lastError: IOException? = null

        for (url in target.urls) {
            try {
                temp.delete()   // 清掉上一次尝试留下的残片
                internetManager.downloadToFile(url, temp)

                if (!DownloadIntegrity.matches(temp, target.sha1, target.sha512)) {
                    temp.delete()
                    lastError = IOException(
                            "hash mismatch (${DownloadIntegrity.algorithm(target.sha1, target.sha512)}) for $url")
                    if (target.urls.size > 1) {
                        LOGGER.warn("Hash mismatch, discarding download and trying next link for $modName ($url)")
                    } else {
                        LOGGER.warn("Hash mismatch, discarding download for $modName ($url)")
                    }
                    continue
                }
                ...
                LOGGER.info("[" + String.format("% 3d", counter.incrementAndGet()) + "/" + totalCount + "] Downloaded mod: " + modName +
                        (DownloadIntegrity.algorithm(target.sha1, target.sha512)?.let { " (verified $it)" } ?: ""))
                return
    ...
    /**
     * 把校验通过的临时文件移到最终路径。同目录内优先原子移动，失败（跨卷/文件系统不支持）再退化为替换式移动。
     */
    private fun moveIntoPlace(temp: File, dest: File) {
    ...
    companion object {
        /** 下载中的临时文件后缀（不以 .jar 结尾，因此不会被客户端模组扫描当成模组）。 */
